...
USTT_UseGameplayTask* USTT_UseGameplayTask::UseSmartObjectWithGameplayInteraction(AAIController* Controller, const FSmartObjectClaimHandle ClaimHandle, const bool bLockAILogic)
{
	...
	USTT_UseGameplayTask* MyTask = NewAITask<USTT_UseGameplayTask>(*Controller, EAITaskPriority::High);
	if (MyTask == nullptr)
	{
		return nullptr;
	}
...
USTT_UseGameplayTask* USTT_UseGameplayTask::MoveToAndUseSmartObjectWithGameplayInteraction(AAIController* Controller, const FSmartObjectClaimHandle ClaimHandle, const bool bLockAILogic)
{
	USTT_UseGameplayTask* NewTask = UseSmartObjectWithGameplayInteraction(Controller, ClaimHandle, bLockAILogic);
	if (NewTask != nullptr)
	{
...
//----------------------------------------------------------------------//
USTT_UseGameplayTask::USTT_UseGameplayTask(const FObjectInitializer& ObjectInitializer)
	: Super(ObjectInitializer)
{
	bTickingTask = true;
}

void USTT_UseGameplayTask::TickTask(const float DeltaTime)
{
...
void USTT_UseGameplayTask::Activate()
{
	Super::Activate();
	...
	SmartObjectSubsystem->RegisterSlotInvalidationCallback(ClaimedHandle, FOnSlotInvalidated::CreateUObject(this, &USTT_UseGameplayTask::OnSlotInvalidated));
...
bool USTT_UseGameplayTask::StartInteraction()
{
...
void USTT_UseGameplayTask::OnGameplayTaskDeactivated(UGameplayTask& Task)
{
	if (MoveToTask == &Task)
	{
...
void USTT_UseGameplayTask::OnDestroy(const bool bInOwnerFinished)
{
...
void USTT_UseGameplayTask::OnSlotInvalidated(const FSmartObjectClaimHandle& ClaimHandle, ESmartObjectSlotState State)
{
	Abort(EGameplayInteractionAbortReason::InternalAbort);
}

void USTT_UseGameplayTask::Abort(const EGameplayInteractionAbortReason Reason)
{
...
void USTT_UseGameplayTask::RequestAbort()
{
